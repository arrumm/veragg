...
@Service
public class CrawlerJobRunnerServiceImpl implements CrawlerJobRunnerService {

    private static final Logger LOGGER = LoggerFactory.getLogger(CrawlerJobRunnerServiceImpl.class);
    ...
    @Autowired
    public CrawlerJobRunnerServiceImpl(ApplicationContext applicationContext, TaskScheduler taskScheduler) {
    ...
    private void schedule(final Collection<CrawlerJobWrapper<CrawlerJob<?>>> crawlerJobWrapperCollection) {
        for (final CrawlerJobWrapper<CrawlerJob<?>> crawlerJobWrapper : crawlerJobWrapperCollection) {
            ...
            taskScheduler.schedule(() -> asyncRun(crawlerJobWrapper), new CronTrigger(crawlerJobWrapper.getConfiguration().getSchedule()));
        }
    }

    private void asyncRun(final CrawlerJobWrapper<CrawlerJob<?>> crawlerJobWrapper) {
        LOGGER.info("asyncRunJob(jobWrapper = [{}])", crawlerJobWrapper);

        asyncRun(crawlerJobWrapper.getJob());
    }

    private void asyncRun(CrawlerJob<?> job) {
        executorService.submit(() -> {
            try {
                job.execute();
            } catch (final Exception e) {
                LOGGER.error("asyncRunJob.call(): Error while executing job {}: {}", job.getId(), e.getMessage(), e);
            }
        });
    }
    ...
    @Override
    public void run(CrawlerJob<? extends Crawling> job) {
        asyncRun(job);
    }
}
